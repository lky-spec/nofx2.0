	...
	case "exchange_management":
		...
		if lang == "zh" {
			return strings.Join([]string{
				...
				"- 每个交易所的必填凭证以产品字段目录为准；账户显示名只是本地显示名，凭证字段不要混用。",
				"- 关键字段：" + strings.Join(fields, "、"),
		...
		return strings.Join([]string{
			...
			"- Required credentials are defined by the product field catalog for each exchange; do not mix display names with credential fields.",
			"- Key fields: " + strings.Join(fields, ", "),
